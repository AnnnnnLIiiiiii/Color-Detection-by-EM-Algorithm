...
images = load_images_from_folder("green")
...
green_gmm = em.GaussianMixture(train, 1)
hist_br = np.sum(hist_bgr, axis=1)
# br_gmm = em.GaussianMixture(hist_br, 1)
iteration = 200
for _ in range(iteration):
    green_gmm.train()
print("likelihood: ", green_gmm.getLikelihood()-len(train)*3*np.log(255))
...
while success:
    ...
    print('Gaussian mean: ', green_gmm.getModel()[0][0]*255)
    prob = green_gmm.getPdf(tmp)
    # prob = em.gaussian(tmp, green_gmm.getModel()[0][0], green_gmm.getModel()[1][2]).reshape((-1))
    ...
    u, s, vh = np.linalg.svd(green_gmm.getModel()[1][0]*255**2)
    print("eigen value, ", s)
    print("eigne vector", vh)


    #_, img_thresh = cv2.threshold(prob, prob.mean()*2, 255, cv2.THRESH_BINARY)
    _, img_thresh = cv2.threshold(prob, 20, 255, cv2.THRESH_BINARY)
    _, img_thresh_slack = cv2.threshold(prob, 10, 255, cv2.THRESH_BINARY)
