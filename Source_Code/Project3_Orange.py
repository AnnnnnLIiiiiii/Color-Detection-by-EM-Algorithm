...
images = load_images_from_folder("orange")
...
orange_gmm = em.GaussianMixture(train, 4)
hist_br = np.sum(hist_bgr, axis=1)
# br_gmm = em.GaussianMixture(hist_br, 1)
iteration = 200
for _ in range(iteration):
    orange_gmm.train()
print("likelihood: ", orange_gmm.getLikelihood()-len(train)*3*np.log(255))
...
while success:
    ...
    print('Gaussian mean: ', orange_gmm.getModel()[0][0]*255)
    #prob = orange_gmm.getPdf(tmp)
    prob = em.gaussian(tmp, orange_gmm.getModel()[0][2], orange_gmm.getModel()[1][2]).reshape((-1))
    prob2 = em.gaussian(tmp, orange_gmm.getModel()[0][2], orange_gmm.getModel()[1][2]).reshape((-1))


    prob = np.reshape(prob, (img_shape[0], img_shape[1]))
    prob2 = np.reshape(prob2, (img_shape[0], img_shape[1]))

    print('prob max, ', prob.max()/(255**3))
    print("prob sum:", np.sum(prob)/(255**3))
    print("prob mean", np.mean(prob))
    # print("prob cov", orange_gmm.getModel()[1][0]*255**2)
    u, s, vh = np.linalg.svd(orange_gmm.getModel()[1][0]*255**2)
    print("eigen value, ", s)
    print("eigne vector", vh)


    #_, img_thresh = cv2.threshold(prob, prob.mean()*2, 255, cv2.THRESH_BINARY)
    _, img_thresh = cv2.threshold(prob, 10e-15, 255, cv2.THRESH_BINARY)
    # _, img_thresh_slack = cv2.threshold(prob2, 10e-4, 255, cv2.THRESH_BINARY)
    _, img_thresh_slack = cv2.threshold(prob2, 10e-50, 255, cv2.THRESH_BINARY)
    ...
    cv2.rectangle(mask, (x - int(0.5 * w), y - h), (x + int(1.5 * w), y + int(1.5 * h)), 255, -1)
